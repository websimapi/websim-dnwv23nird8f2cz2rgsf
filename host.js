...
export async function initHost(room, dataDisplayEl) {
    ...
    function cleanDisconnectedPlayers() {
        ...
        for (const userId in playersData) {
            if (!connectedPeerUserIds.has(userId)) {
                // Player is disconnected. Instead of deleting, we could mark them as inactive,
                // but for now, we'll just leave their data as-is to preserve their last position.
                // The client-side will simply stop receiving updates for them, so they will appear frozen.
                // console.log(`Player ${playersData[userId]?.username} (${userId}) is disconnected. Their data will be preserved.`);
            }
        }
        // No update needed here anymore based on disconnections, as we are not changing the data.
    ...
    setInterval(() => {
        ...
        // Broadcast the current player state to all clients
        room.send({
